    ...
    onPunchIn() {
      ...
      wx.getLocation({
        ...
        success: (res) => {
          ...
          // 调用逆地理编码接口，获取城市信息
          that.reverseGeocode(latitude, longitude, (city) => {
            // 去掉“市”字
            ...
            wx.request({
              ...
              data: {
                location: formattedCity, // 将处理后的城市信息存入数据库
              },
              success: function (res) {
      ...
      wx.request({
        ...
        success: (res) => {
          if (res.data.status === '1') {
            const city = res.data.regeocode.addressComponent.city; // 获取城市信息
            callback(city); // 返回城市信息
          } else {
